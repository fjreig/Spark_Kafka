...
### 7. Almacenamos los datos de la query en un nuevo topic de Kafka

df2 = df1.select("date", to_json(struct("EA_red", "ES_red", "EA_gen", "FP_red", "EA_Consumo")).alias("value"))
df2 = df2.withColumnRenamed("date", "key")
df2 = df2.withColumn("key", df2.key.cast("string"))

df2.write \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "redpanda:9092") \
    .option("topic", "FV_limpio") \
    .save()
